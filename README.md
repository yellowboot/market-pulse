...
Сайт обновляется сам, без участия человека: GitHub Actions по расписанию запускает `fetch_news.py` (раз в час в рабочие часы рынка по будням, дважды в день по выходным — см. `.github/workflows/update-news.yml`), коммитит свежий `news_data.js` обратно в репозиторий, а GitHub Pages тут же отдаёт обновлённую страницу. Открывая сайт в браузере, вы просто видите готовый результат — ничего запускать вручную не нужно.
